...
# Independent Liquidity Testing Setup Script for CoW Protocol with Balancer Solver - AVALANCHE
# This script sets up and runs the autopilot, driver (NO LIQUIDITY), liquidity-driver, and balancer solver

set -e

# Configuration for Avalanche
NODE_URL="https://api.avax.network/ext/bc/C/rpc"  # Avalanche RPC
ORDERBOOK_URL="https://barn.api.cow.fi/avalanche"  # Avalanche orderbook
DRIVER_PORT="11088"
LIQUIDITY_DRIVER_PORT="8080"  # Liquidity-driver port
SOLVER_PORT="8083"  # Different port to avoid conflicts
CHAIN_NAME="avalanche"

echo "🚀 Setting up CoW Protocol Independent Liquidity Testing - AVALANCHE"
echo "============================================================"
echo "Chain: $CHAIN_NAME (Chain ID: 43114)"
...
# Setup avalanche configuration
echo "🔧 Setting up avalanche configuration..."
ln -sf configs/avalanche/driver.toml driver.config.toml
ln -sf configs/avalanche/baseline.toml baseline.config.toml
ln -sf configs/avalanche/liquidity-driver.toml liquidity-driver.config.toml

echo "✅ Avalanche configuration loaded"
...
echo ""
echo "✅ All services started successfully for AVALANCHE INDEPENDENT LIQUIDITY TESTING!"
echo ""
echo "📊 Service Status:"
echo "   - Chain: $CHAIN_NAME (Chain ID: 43114)"
...
echo "     -d '{\"auctionId\":123,\"tokens\":[],\"tokenPairs\":[[\"0xB31f...\",\"0xB97E...\"]],\"blockNumber\":18000000,\"protocols\":[\"balancer_v2\"]}'"
echo ""
